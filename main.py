...
def main():
    ...
    write_file('data/output.txt', console_text)

    file_text = read_file('data/output.txt')
    output_text(file_text)
    write_file('data/text_from_file.txt', file_text)

    new_row = pandas.DataFrame({'Column1': [console_text]})
    write_file_pandas('data/new_csv_file.csv', new_row)

    df_from_file = read_file_pandas('data/new_csv_file.csv')
    df_text = str(df_from_file)
    output_text(df_text)
    write_file('data/pandas_read_output.txt', df_text)
